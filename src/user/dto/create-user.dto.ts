...
export class CreateUserDto {
    ...
    @ApiProperty({example:'테스트닉네임', description:'회원 닉네임'})
    @IsString()
    readonly user_nickname: string | null;

    @ApiProperty({example:'01012341234', description:'회원 전화번호'})
    @IsString()
    readonly user_phone: string | null;

    @ApiProperty({example:'123456', description:'회원 우편번호'})
    @IsString()
    readonly user_postcode: string | null;

    @ApiProperty({example:'테스트주소', description:'회원 주소'})
    @IsString()
    readonly user_add: string | null;

    @ApiProperty({example:'테스트상세주소', description:'회원 상세주소'})
    @IsString()
